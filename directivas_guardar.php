...
if (isset($perfil_id)){
        ...
        if ($dir_id == 1){            
            //echo 'llego';
            $query = "INSERT INTO arg_usuarios_privilegios (perfil_id, directiva_id, valor, u_id_edit, fecha_created) ".
                     "VALUES(".$perfil_id.",".$dir_id.",".$valorm.",".$u_id.",'".$fecha."')";
         //echo $query; die();
            $mysqli->query($query);
            
            $datos_menus = $mysqli->query("SELECT menu_id, menu FROM arg_sys_menus 
                                           WHERE menu_id NOT IN (SELECT DISTINCT valor 
                                                                 FROM arg_usuarios_privilegios 
                                                                 WHERE 
                                                                    directiva_id = 1 AND perfil_id = ".$perfil_id.")") or die(mysqli_error());
            $html.=  "<table class='table text-black' id='datos_dir'>
                 ...
                 while ($res = $datos_menus->fetch_assoc()) {
                        ...
                        $menu    = $res['menu'];                           
                        $html.="<tr>
                                   <td style='display:none;' id='perfil_id".$c."'>".$perfil_id."</td> 
                                   <td style='display:none;' id='dir_id".$c."'>".$dir_id."</td> 
                                   <td style='display:none;' id='user_id".$c."'>".$user_id."</td> 
                                   <td>".$menu."</td>
                                   <td><div class='form-check'>
                                    <input type='checkbox' class='form-check-input' id='menu".$c."'>
