...
var $htmlBeforePageText = '';

var $footer_margin = -130;

public function HTML2FPDF($orientation='P',$unit='mm',$format='Letter')
{
...
function Footer() {
	...
	$this->Cell(0,10,$this->htmlBeforePageText,0,0,'C');
	...
	foreach($a as $i => $e) {
		if($i%2 == 0) {
			...
			elseif ($this->specialcontent) {
				...
				if ($e != '') {
					$this->Write($this->lineheight, $e, $this->x); //Write text directly in the PDF
...
// What this gets: < $tag $attr['WIDTH']="90px" > does not get content here </closeTag here>
  ...
  $this->blockjustfinished=false;
	...
	case 'CFS':
		...
		//$this->Write($this->lineheight, $attr['FONTSIZE']);
		break;
	case 'CXY':
		if (isset($attr['X'])) {
			$this->SetXY($attr['X'], $this->y);
		}
		if (isset($attr['Y'])) {
			$this->SetXY($this->x, $attr['Y']);
		}
    ...
    if(!$this->pbegin and !$this->divbegin and !$this->tablestart)
    {
      if($this->cssbegin)
      {
          ...
          if ($this->cssbegin and ($this->divborder or $this->dash_on or $this->dotted_on or $this->divbgcolor))
          {
              ...
              $tempx = $this->x;
              if($this->divbgcolor) $this->Cell($this->GetStringWidth($texto),$this->lineheight,'',$this->divborder,'','L',$this->divbgcolor, "", $this->x);
...
//! @desc Prints all list-related buffered info
    ...
    foreach($this->listitem as $item)
    {
        ...
        //Output bullet
        $this->Cell($blt_width,5,$type,'','','L', '0', "", $this->x);
...
//! @desc Disable some tags using ',' as separator. Enable all tags calling this function without parameters.
  if ($str == '') //enable all tags
  {
    ...
    $this->enabledtags = "<tt><kbd><samp><option><outline><span><newpage><page_break><s><strike><del><bdo><big><small><address><ins><cite><font><center><sup><sub><input><select><option><textarea><title><form><ol><ul><li><h1><h2><h3><h4><h5><h6><pre><b><u><i><a><img><p><br><strong><em><code><th><tr><blockquote><hr><td><tr><table><div><cfs><cxy>";
  }
  else
  {
    $str = explode(",",$str);
