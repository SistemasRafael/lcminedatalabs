    ...
    $sheet = $spreadsheet->getActiveSheet();
     ...
     if ($result = mysqli_store_result($mysqli)) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                //$row_count = $row['folio_interno'];
               //DGO cada 10 if ($bloque%$total_bloque == 0){
            if ($i == 1){
                    ...
                    $i=$i+1;
                    $sheet->setCellValue('A'.$i, $i);//->setValue($i);
                    $sheet->setCellValue('B'.$i, 'REAGENT BLANK');//->setValue('BLANCO REACTIVO');
           ...
           //DGO if ($cont%$bloque6 == 0 && $bloque%$total_bloque <> 0){
            if ($cont%$bloque6 == 0 && $row_count < $i){
                    $sheet->setCellValue('A'.$i, $i);
...
$fileName = $folio.'_'.$metodo_id_a.'-ETI.csv';
