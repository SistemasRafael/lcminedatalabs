...
IF (isset($trn_id_a)){
    ...
    $orden_encabezado = $datos_orden->fetch_assoc(); 
        ...
        mysqli_multi_query ($mysqli, "CALL arg_prc_liberarResultados_sobr (".$trn_id_a.", ".$metodo_id_a.", ".$u_id_a.", ".$unidad_id_ree.")") OR DIE (mysqli_error($mysqli));             
    
     if($result = mysqli_store_result($mysqli)){
		mysqli_free_result($result);
    } while(mysqli_more_results($mysqli) && mysqli_next_result($mysqli));
    
     mysqli_multi_query ($mysqli, "CALL arg_consultar_resultados_sobr (".$trn_id_a.", ".$metodo_id_a.", 2".")") OR DIE (mysqli_error($mysqli));
