...
$datos_atiende = $mysqli->query("SELECT DISTINCT usu.email AS atiende_correo, usu.nombre 
                            ...
                            WHERE ene.estado_id = 4 AND ent.fecha_inicio = '".$hoy."'")  or die(mysqli_error());
...
$mail = new PHPMailer();
    ...
    while ($fila_correos = $datos_atiende->fetch_assoc()) {
        ...
        $datos_v = $mysqli->query("SELECT ene.trn_id, folio, ene.fecha AS fecha_solicitado, ent.fecha_inicio AS fecha_visita, usu.nombre, org.nombre AS empresa
                                ...
                                WHERE ene.estado_id = 4 AND ent.fecha_inicio = '".$hoy."' AND ati.email = '".$atiende_correo."'")  or die(mysqli_error());

				//include("crearPdfimagen.php"); 
				
				$mail->From = "bitacora.arg@argonautgold.com";
				$mail->FromName = "Bitacora de Registro";
				$mail->Subject = "Visitas iniciadas";		
                $mail->AddBCC("".$atiende_correo."");	
				//$mail->AddBCC("".$atiende_correo."");
        
				$mail->ContentType = "text/html";
				$body = $atiende_nombre." "."usted ha iniciado hoy <strong>".$hoy."</strong> las siguientes visitas, si requiere más tiempo favor de cambiar el estado a <strong>prolongado</strong> o bien dar por <strong>terminada</strong> la visita.<br><br>";	
