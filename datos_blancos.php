...
$minimo = $_POST['minimo'];
    ...
    $max_metodo_id = $mysqli->query("SELECT MAX(material_id) AS material_id FROM arg_controles_blancos") or die(mysqli_error());
    $max_meto = $max_metodo_id ->fetch_array(MYSQLI_ASSOC);
    $material_id = $max_meto['material_id'];
    $material_id = $material_id+1;
    
if (isset($u_id)){
    $query = "INSERT INTO arg_controles_blancos (material_id, control_id, valor_ley, maximo, minimo, metodo_id, u_id) ".
             "VALUES ($material_id, $control_id, $ley, $maximo, $minimo, $metodo_id, $u_id)";
             $mysqli->query($query) ;
                            //echo $query;
                            //die();
