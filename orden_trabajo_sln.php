...
<script>
    ...
    function verificar_seleccion(numb){          
     ...
     function agregarFila(){
          ...
          '<td><select name="'+name_banco+'" id="'+name_banco+'" onchange=actualiza_vol('+contador+') class="form-control">' 
                    <?$result = $mysqli->query("SELECT 0 AS banco_id, 'Seleccione' AS banco UNION ALL SELECT banco_id, banco FROM arg_bancos WHERE unidad_id = ".$_SESSION['unidad_id']) or die(mysqli_error());                             
                              while ( $row1 = $result ->fetch_array(MYSQLI_ASSOC)) {
                                $banco = $row1['banco'];                                
                    ?>       
        +'<option value="<?echo $row1['banco_id']?>"><?echo $banco?></option>'
        <?}?>
        +'</select></td>'
        +'<td><select name="'+name_voladura+'" id="'+name_voladura+'" class="form-control">' 
                    <?$result = $mysqli->query("SELECT 0 AS voladura_id, 'Seleccione' AS vol UNION ALL SELECT voladura_id, voladura_id AS vol FROM arg_bancos_voladuras") or die(mysqli_error());                             
                              while ( $row1 = $result ->fetch_array(MYSQLI_ASSOC)) {
                                $vol = $row1['vol'];                                
                              ?>       
                                +'<option value="<?echo $row1['voladura_id']?>"><?echo $vol?></option>'
                             <?}?>
        +'</select></td>'
        ...
        if(($_SESSION['LoggedIn']) <> '')
        {
            ...
            if (isset($_GET['trn_id'])){
                ...
                if (isset($_POST['generar_orden'])){        
                     ...
                     while($i<>0){
                        $banco = 'banco'.$pos;
                        $banco_sel = $_POST[$banco];
                        $voladura = 'voladura'.$pos;
                        $voladura_sel = $_POST[$voladura];
                        ...
                        if ($banco_sel == 0 and $voladura_sel == 0 and $cantidad_sel == '' and $val_met == 0){
                                $i = 0;
                            }
                        elseif ($val_met == 0){                            
                                    echo "<script>";
                        ...
                        elseif ($banco_sel == 0 || $voladura_sel == 0 || $cantidad_sel == ''){
                                    echo "<script>";
                            ...
                            if ($i == 1){
                                 ...
                                 //Ordenes
                                 $query = "INSERT INTO arg_ordenes (trn_id, folio, hora, fecha_inicio, fecha_final, unidad_id, usuario_id, activo, comentario ) ".
                                          "VALUES ($trn_id, $folio_orden, '$hora', '$fecha', '', $unidad_id, $u_id, 1, '')";
                                 $mysqli->query($query) ;
                                 //echo $query;
                         }
                             
                             //Insertamos datos                        
                             $banco_is = $mysqli->query("SELECT banco FROM arg_bancos WHERE banco_id = ".$banco_sel." AND unidad_id = ".$unidad_id) or die(mysqli_error());
                             $banco_ins = $banco_is ->fetch_array(MYSQLI_ASSOC);
                             $banco = $banco_ins['banco'];
                             
                             //VOLADURAS                                 
                             $max_con_id = $mysqli->query("SELECT folio_actual FROM arg_bancos_voladuras WHERE banco_id = '".$banco_sel."' AND voladura_id = ".$voladura_sel) or die(mysqli_error());
                             ...
                             $length_c = 3;
                             $string_c = (string)$cons;
                             $cons_c = str_pad($string_c,$length_c,"0", STR_PAD_LEFT);
                             
                             $folio_inicial = $caracter_folio.$banco.$voladura_sel.$folio_actual_sig;
                             $folio_final   = $caracter_folio.$banco.$voladura_sel.$folio_actual_fin;
                             $folio_interno = $serie_mina.$folio_orden_int.'-'.$cons_c;
                             ...
                             $query = "INSERT INTO arg_ordenes_detalle (trn_id, trn_id_rel, banco_id, voladura_id, cantidad, folio_inicial, folio_final, folio_interno, folio_interno_final, estado, usuario_id) ".
                                      "VALUES ($tr_id_det, $trn_id, $banco_sel, $voladura_sel, $cantidad_sel, '$folio_inicial','$folio_final', '$folio_interno', '', 0, $u_id)";
                             $mysqli->query($query) ;
                             //echo $query
                             ...
                             $c = 1;
                                 while ($c <= $cantidad_sel){
                                     ...
                                     $folio_inicial_det = $caracter_folio.$banco.$voladura_sel.$folio_actual_det;
                                     $folio_interno_det = $serie_mina.$folio_orden_int.'-'.$cons_deta;
                                     
                                    $query = "INSERT INTO arg_ordenes_muestras (trn_id, trn_id_rel, folio, folio_interno) ".
                                             "VALUES ($trn_id_mue, $tr_id_det, '$folio_inicial_det', '$folio_interno_det')";
                                    $mysqli->query($query) ;
                                    //echo $query;
                                    $c++;
                                    $trn_id_mue++;
                    ...
                    <fieldset>                       
                            <div class="col-md-12 col-lg-12 bg-warning text-black text-center">
                                <br />
                                    ...
                                    <div class='col-sm-2'>
                                         ...
                                         <!--<div class="form-group">
                                            ...
                                            <div class='input-group date' id='datetimepicker3'>
                                               ...
                                               <input type='text' id='hora_sel' class="form-control" />
                                     ...
                                     <td>
                                         ...
                                         <div class="[ form-group ] ">   
                                            <?while ($fila = $datos_res->fetch_assoc()) {?>
                                                    ...
                                                    <div class="[ btn-group ]">                                                                
                                                        <label for="<?echo 'fila1_'.$fila['metodo_id']?>" class="[ btn btn-warning ]">
