...
<script>
  ...
  function iniciar_etapa(trn_id, etapa, unidad)
   ...
   function peso_guardar()
    {
         var j = 1;   
         var existe_cero = 0;
         var table = document.getElementById("tabla_peso");
            ...
            if(total_porc_hum < 0){
                ...
                $('#boton_save_hum').html('<div class="loading"><i class="fa fa-spinner fa-spin fa-1x fa-fw"></i><span class="sr-only">Loading...</span></div>');
                    $.ajax({
                        ...
                        //if(respuesta == 'El método ha finalizado.')//alert(respuesta);  
                       // {
                       //     alert(respuesta);
                      //  }
                      //  else
    ...
    function quebrado_guardar(trnid_batch, trnid_rel, contador)
    {
         ...
         var total_rows_q = parseInt(table.rows.length)-1;
         if (total_rows_q == c){
             var fin = 1;
         }
         else{
         ...
         comentario_que = document.getElementById(com_que).value;
          ...
          if(total_porc_que == 0 || total_porc_que == '' || total_pesomalla_que == '' )
                        {
                            error_det = 'Error: el porcentaje no puede ser cero. Reintente Línea 1.';
                            warning_0 = 1;                          
                        }
        
         if  (total_porc_que < 70 && comentario_que == '' && total_porc_que != '')  {   
              warn_porc = 1;
    ...
    function pulverizado_guardar(trnid_batch, trnid_rel, contador)
    {
         ...
         var total_rows_p = parseInt(table.rows.length)-1;
         if (total_rows_p == c){
             var fin = 1;
         }
         else{
         ...
         if(total_porc_pul < 85 && comentario_pul == '' && total_porc_pul != '')
            {
                warn_porc = 1;
                error_det = 'Si el porcentaje es menor que 85 debe especificar un comentario.';
            }      
    ...
    function iniciar_metodo(trn_id, metodo, fase, etapa, unidad)
    {
            ...
            if (etapa == 11){
               enviar_resultados(trn_id, metodo, unidad);
            }
    ...
    function met_peso_guardar(trnid_batch, trnid_rel, metodo, fase, etapa, contador, unidad)
    {
         ...
         var total_rows_m = parseInt(table.rows.length)-2;
         //alert(contador);
         //$('#guardando_modal_peso').modal('show');
         //alert(total_rows_m);
    ...
    function met_payon_guardar(trnid_batch, trnid_rel, metodo, fase, etapa, contador, unidad)
    {
         ...
         var total_rows_pay = parseInt(table.rows.length)-2;
    ...
    function copelado_guardar(trn_id_cop, metodo_cop)
        {               
            ...
            {
                if(cantidad_cp < 840 || cantidad_cp > 880){
                    alert('Error: Temperatura fuera de rango, reintente por favor');
                }
                else{
    ...
    function digestion_guardar(trn_id_dig, metodo_dig)
        {         
            ...
            {
                if(cantidad_dg < 65 || cantidad_dg > 85){
                    alert('Error: Temperatura fuera de rango, reintente por favor');
                }        
                else{
    ...
    function ver_csv(archivo, metodid_importar)
        {         
               ...
               //alert('El archivo se importó satisfactoriamente: '+archivo_imp);
               $.ajax({
                		url: 'leer_csv.php' ,
                		type: 'POST' ,
                		dataType: 'html',
   ...
   <div class="modal-dialog modal-dialog-scrollable" role="document">
            <div class="modal-content">
              ...
              <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" id='boton_save_sec' data-toggle="modal" data-target="#exampleModal" id="peso_btn" onclick="peso_guardar()">Guardar</button>
              </div>
            </div>
    </div>              
</div>
   ...
   <div class="modal-dialog modal-dialog-scrollable" role="document">
            <div class="modal-content">
              <div class="modal-header" >
                ...
                <div class="col-md-1 col-lg-1">                
                    <input type="date" id="fecha_metodo"  value="<?php echo date("Y-m-d");?>" min="<?php echo date("Y-m-d");?>" disabled />                    
                    <input type="hidden" id="mina_payon" size=20 style="width:125px; color:#996633" value="<?php echo $unidad_id;?>" disabled /> 
                </div>
              </div> 
            ...
            <div class="modal-content">
              ...
              if ($action == "upload") //si action tiene como valor UPLOAD haga algo (el value de este hidden es es UPLOAD iniciado desde el value
                ...
                //cargamos el archivo al servidor con el mismo nombre(solo le agregue el sufijo bak_)            
                    ...
                    $dest  = '/var/www/html/MineData-Labs/absorcion'.'/ '; //lugar donde se copiara el archivo
                  ...
                  $html_det = "<table class='table table-striped' id='motivos'>
                                <thead>                                
                                    ...
                                    <tr class='table-info' justify-content: center;>            
                                        ...
                                        <th scope='col1'>Muestras</th>
                                        <th scope='col1'>Etiquetas</th>";             
                                    $html_det.="</tr>
                               </thead>
                               <tbody>";
                               ...
                               while ($fila = $datos_orden_detalle->fetch_assoc()) {
                                   ...
                                   $html_det.="<tr>";
                                      ...
                                      if ($fila['reensaye'] == 1){
                                            $html_det.="<td> <a button type='button'class='btn btn-success' href='orden_trabajo_muestras_ree.php?trn_id=".$fila['trn_id_batch']."&metodo_id=".$fila['metodo_id']."&unidad_id=".$unidad_id."&ree=1' target='_blank'>
