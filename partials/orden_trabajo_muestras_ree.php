   ...
   <script>
       ...
       function redireccion(trn_id, unidad_id) {
           ...
           //alert(metodo_id);
           //window.history.back();
           var direccionar = '<?php echo "\ orden_trabajo_muestras_ree.php?trn_id=" ?>' + trn_id + '&metodo_id=' + metodo_id + '&unidad_id=' + unidad_id + '&ree=1';
       ...
       <div class="container">
           <div class="container">
               ...
               <div class="col-md-4 col-lg-4">
                ...
                $html_det .= "<th colspan='3'> Método: " . $datos_batch['metodo'] . "</th>";
                ...
                /*$muestras_det = $mysqli->query("SELECT
                                                                    ...
                                                                    ot.control
                                                                FROM
                                                               	     arg_muestras_reensaye ree
                                                                    	LEFT JOIN ordenes_reensayes ot
                                                                            ON  ree.trn_id_batch = ot.trn_id_batch
                                                                            AND ree.trn_id_muestra = ot.trn_id_muestra
                                                                            AND ree.metodo_id = ot.metodo_id
                                                                WHERE 
                                                                	ree.trn_id_rel = ".$trn_id." AND ree.metodo_id = ".$metodo_id."
                                                                ORDER BY
                                                                	ot.muestra_geologia"
                                                                ) or die(mysqli_error());
                                } Cambio el 09 Julio por reensayes de reensayes*/
                ...
                $muestras_sobrelimites = $mysqli->query("SELECT
                    sl.trn_id_batch,
                    sl.trn_id_muestra,
                    ade.trn_id_rel,
                ...
                WHERE
                    sl.trn_id_rel = $trn_id") or die(mysqli_error($mysqli));

                if (mysqli_num_rows($muestras_sobrelimites) > 0) {
                    $html_det .= "<tr class='table-secondary' justify-content: left;>
                                        <th scope='col1'>Batch</th>
                                        <th scope='col1'>Trn ID</th>                             
                                        <th scope='col1'>Muestra</th>
                                        <th scope='col1'>Control</th></tr>";
                    while ($muestras = $muestras_sobrelimites->fetch_assoc()) {

                        $html_det .= "<tr>";
                        $html_det .= "<td>" . $muestras['trn_id_batch'] . "</td>";
                        $html_det .= "<td>" . $muestras['trn_id_muestra'] . "</td>";
                        $html_det .= "<td>" . $muestras['muestra'] . "</td>";
                        $html_det .= "<td>" . $muestras['control'] . "</td>";
                        $html_det .= "</tr>";
                        $i++;
                    }
                }
