...
<script>
  ...
  function notificar_nuevoMR(material) {
    var material_ref = material;
    //alert(material_ref);
    $.ajax({
            		url: 'notificacion_nuevoMR.php' ,
            		type: 'POST' ,
            		dataType: 'html',
            		data: {material_ref},
            	})
  ...
  function GuardarMateriales(unidad_id) {
    ...
    var carbonesricos = document.getElementById("carbones").value;
    var unidad_id = unidad_id
    //alert(metodo_id);
    $.ajax({
    ...
    <form name='importar_veh' method='post' action='controles.php?unidad_id=<?echo $unidad_id."'";?> enctype='multipart/form-data' >
        ...
        <table width="470" border="0" cellpadding="1" cellspacing="1" class="box">
          ...
          <tr>
            ...
            <th>IMPORTAR CERTIFICADO DE MRCs</th>
            <br />
            <th> </th>
      ...
      <div class="container" class="col-md-12 col-lg-12">
        ...
        <? $datos_materiales = $mysqli->query(
          "SELECT 
                        ...
                        ,(CASE WHEN mat.ricos_pobres = 1 THEN 'Pobres' WHEN mat.ricos_pobres = 0 THEN 'Ricos' ELSE 'No Aplica' END) AS ricospobres
            ...
            <!--  <button type='button' class='btn btn-primary' name='agregar_material' id='agregar_material' data-toggle="modal" data-target="#ModalMaterial" >+ ASIGNAR MATERIAL</button>
                        ...
                        <br/><br/>
                               ...
                               <thead>
                                   <tr class='table-info' justify-content: center;>                                        
                                       ...
                                       <th scope='col1'>Máximo</th>
                                        ...
                                        <th scope='col1'>Carbones</th>
                                       <th scope='col1'>Activo</th>
                                       <th scope='col1'>Cambiar Estado</th>";
                            $html_det .= "</tr>
                              </thead>
                              <tbody>";
                            while ($fila = $datos_materiales->fetch_assoc()) {
                              ...
                              $html_det .= "<td>" . $fila['ricospobres'] . "</td>";                              
            ...
            <br />
                    <? $datos_materiales = $mysqli->query(
                                                            "SELECT 
                                                                ...
                                                                ,(CASE WHEN met.ricos_pobres = 1 THEN 'Pobres' WHEN met.ricos_pobres = 0 THEN 'Ricos' ELSE 'No Aplica' END) AS ricospobres
        ...
        <div id="menu2" class="tab-pane fade">              
                <div id="content" class="col-md-12 col-lg-12">            
                    <br />
                    <br />                  
                    <? $datos_blancos = $mysqli->query("SELECT
            	                                           bl.nombre, un.nombre AS unidad_mina
                                                          ,maximo, minimo
                            ...
                            <table class='table table-striped' id='motivos'>
                                    <thead>
                                        <tr class='table-info' justify-content: center;>            
                                            ...
                                            <th scope='col1'>Maximo Au</th>
                                            <th scope='col1'>Minimo Au</th>                                            
                                            <th scope='col1'>Maximo Ag</th>
                                            <th scope='col1'>Minimo Ag</th>
                                            <th scope='col1'>Activo</th>
                                            <th scope='col1'>Usuario</th>";
                    ...
                    while ($fila = $datos_blancos->fetch_assoc()) {
                      ...
                      $html_det .= "<td>" . $fila['maximo'] . "</td>";
                      $html_det .= "<td>" . $fila['minimo'] . "</td>";                      
                      $html_det .= "<td>" . $fila['maximo_ag'] . "</td>";
                      $html_det .= "<td>" . $fila['minimo_ag'] . "</td>";
                      $html_det .= "<td>" . $fila['activo'] . "</td>";
                      $html_det .= "<td>" . $fila['usuario'] . "</td>";
                      $html_det .= "</tr>";
                    }
                            ...
                            <table class='table table-striped' id='motivos'>
                                    <thead>
                                        <tr class='table-info' align='left';>            
                                            ...
                                            <th scope='col1'>Ley de Corte</th>
                                            <th scope='col1'>% RSD</th>";
                    $html_det .= "</tr>
                                   </thead>
                                   <tbody>";
...
if (isset($_POST['subir_poliza'])) {
  ...
  if($archivo == ''){
     echo ('<script>error_importar('.$unidad_id.')</script>');
      ...
      $desti = 'upload/'.$serie_m.'/'.$archivo;//$_POST['pol_vehi'];
    ...
    //  echo 'entro';
     ...
     // echo 'entro'.$material_id;
      if (isset($u_id)) {
        //$maximo = ($ley+($desv_esta*$cantidad));
        //$minimo = ($ley-($desv_esta*$cantidad));
        $query = "INSERT INTO arg_controles_materiales (unidad_id, material_id, nombre, control_id, valor_ley, desv_esta, cantidad_desviacion, maximo, minimo, metodo_id, u_id, ricos_pobres, file_path) " .
                 "VALUES ($unidad_id, $material_id, '$nombre', 2, $ley, $desv_esta, $cant_desv_esta, $can_maximo, $can_minimo, $met_asign, $u_id, $ricospobres, '$desti')";
        $mysqli->query($query);
        //echo $query;
      //  die();
        ...
        if ($resultado->num_rows > 0) {
          ...
          echo ('<script>notificar_nuevoMR('.$material_id.')</script>');
        } else {
