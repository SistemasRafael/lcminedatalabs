     ...
     function imprimir($unidad_id,$trn_id)
            ...
            /*$.ajax({
            	...
            	.done(function(respuesta){*/
            	   //alert(respuesta);
                        //alert(print_d);  
                 var print_d = '<?php echo "\orden_trabajo_print.php?trn_id="?>'+trn_id;            
                 window.location.href = print_d;		                  
              //}) 
            }
        ...
        if(($_SESSION['LoggedIn']) <> '')
        {
            ...
            if (isset($_GET['trn_id'])){
                ...
                if (isset($_POST['generar_orden'])){        
                        ...
                        elseif ($banco_sel == 0 || $voladura_sel == 0 || $cantidad_sel == ''){
                            ...
                            /* $folio_inicial = $caracter_folio.$banco.$voladura_sel.$folio_actual_sig;
                             ...
                             $query = "INSERT INTO arg_ordenes_detalle (trn_id, trn_id_rel, banco_id, voladura_id, cantidad, folio_inicial, folio_final, folio, folio_interno, estado, usuario_id) ".
                                      "VALUES ($tr_id_det, $trn_id, $banco_sel, $voladura_sel, $cantidad_sel, '$folio_inicial','$folio_final',$folio_det , '$folio_interno', 0, $u_id)";
                             $mysqli->query($query) ;
                             //echo $query;
                             ...
                             $c = 1;
                                 while ($c <= $cantidad_sel){
                                     ...
                                     /*$folio_inicial_det = $caracter_folio.$banco.$voladura_sel.$folio_sig;
                                     $folio_interno_det = $serie_mina.$folio_orden_int.'-'.$cons_deta;*/
                    ...
                    <div class="col-md-1 col-lg-1"> 
                        <input type="submit" class="btn btn-success" name="generar_orden" onclik="validar_hr()" id="generar_orden" data-toggle="modal" data-target="#exampleModal" value="GUARDAR ORDEN" />                      
                    </div>  
