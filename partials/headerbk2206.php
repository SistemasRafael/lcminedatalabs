                ...
                <div class="dropdown-menu">
                      ...
                      if ($_SESSION['unidad_acc'] == '0'){
                         ...
                         while ($row = $datos_at->fetch_assoc()){
                            ?>  <a class="dropdown-item" href="seguimiento_ordenes.php?unidad_id=<?echo $row['unidad_id'];?>"><?echo $row['nombre'];?></a><?
                         }//Fin while
                      ...
                      if ($_SESSION['unidad_acc'] <> '0' AND $_SESSION['unidad_acc'] <> '999'){
                         ...
                         while ($row = $datos_umina->fetch_assoc()){
                            ?><a class="dropdown-item" id="unidad" href="seguimiento_ordenes.php?&unidad_id=<?echo $row['unidad_id'];?>"><?echo $row['nombre'];?></a>
                        <?}
                      }?>
                     <?
                        ...
                        while ($i<=$cadena){
                            ...
                            if (is_numeric($valor)){
                               ...
                               $mina_acc_nombre = $mina_acce['nombre'];
                                ...
                                ?> <a class="dropdown-item" href="seguimiento_ordenes.php?unidad_id=<?echo $mina_acc;?>"><?echo $mina_acc_nombre;?></a><?
                          }
                          $i = $i+1;
                        }
                      }
...
?>
 ...
 <br/>
<div class="col-xl-2 col-sm-1 col-md-3 col-ld-3 ">   
...
<?$mysqli -> set_charset("utf8");
    ...
    while ($row_menu = $datos_menu->fetch_assoc()){ ?>
        <div class="col-xl-3 col-sm-3 col-md-1 col-ld-1 ">
                ...
                <div class="btn-group">
                    <button type="button" id="dropdownMenuButton" class="btn btn-light btn-block dropdown-toggle" data-toggle="dropdown">
                        <?echo $row_menu['nombre_directiva']?>
                             ...
                             <?  $datos_transacciones = $mysqli->query ("SELECT DISTINCT nombre_directiva, valor_directiva
                                                                            menu_id, accion 
                                                                        FROM 
                                                                            `perfiles_privilegios` 
                                                                        WHERE 
                                                                            ...
                                                                            AND u_id = ".$_SESSION['u_id']." ORDER BY  orden") 
                                                                OR DIE(mysqli_error());
                                $total_rows =  mysqli_num_rows($datos_transacciones);
                                $i = 1;
                                while ($row_transaccion = $datos_transacciones->fetch_assoc())
                                {?>                                   
                                       <li> <a href="<?echo $row_transaccion['accion'].$unidad_mina_sel;?>"><h5><?echo $row_transaccion['nombre_directiva']?></h5></a> </li>                                      
                                       <?if ($i < $total_rows){?>
                                            <li class="divider"></li>
                                       <?}?>                                     
                                       <?$i++;?>
