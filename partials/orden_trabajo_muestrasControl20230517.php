   ...
   <script>
       ...
       function exportar_muestras(trn_id, metodo_id) {
           var trn_id = trn_id;
           var metodo_id = metodo_id;
           var exportar = '<?php echo "\ exportar_orden_trabajo_muestrasControl.php?trn_id=" ?>' + trn_id + '&metodo_id=' + metodo_id;
       ...
       function print_muestras(trn_id, metodo) {
           ...
           var exportar = '<?php echo "\ listado_muestras_metodo_pdf.php?trn_id=" ?>' + trn_id_ex + '&metodo_id=' + metodo;
           window.location.href = exportar;
       }
   ...
   <?php
    ...
    echo 'aca'.$metodo;*/
        ...
        $datos_orden = $mysqli->query("SELECT
                                        ...
                                        ,(CASE ord.tipo WHEN 1 THEN 1 WHEN 2 THEN 2 WHEN 5 THEN 5 WHEN 6 THEN 6 ELSE 0 END) AS tipo_orden
                                   FROM 
        ...
        $datos_batchs = $mysqli->query(
            "SELECT trn_id_rel as trn_id_batch, folio_interno, m.nombre as metodo
                                    FROM 
    ...
    ?>
       ...
       <div class="container">
           <div class="container">
               ...
               <div class="col-md-4 col-lg-4">
                   <select name="metodo_id_sel" id="metodo_id_sel" value="<? echo $metodo_id; ?>" onchange="redireccion(<? echo $trn_id ?>, <? echo $metodo_id ?>, <? echo $unidad_id ?>)" class="form-control">
               ...
               <div class="col-4 col-md-4 col-lg-4">
                   <button type="button" class="btn btn-info" name="print_muestras" id="print_muestras" onclick="print_muestras(<? echo $trn_id ?>, <? echo $metodo_id ?>, 1)"> <span class="fa fa-file-pdf-o fa-2x"> Imprimir</span> </button>

                   <button type="button" class="btn btn-success" name="export_muestras" id="export_muestras" onclick="exportar_muestras(<? echo $trn_id ?>,<? echo $metodo_id ?>)"> <span class="fa fa-file-excel-o fa-2x"> Exportar</span> </button>
               </div>
                ...
                if ($metodo_id == 0) {
                    ...
                    if ($orden_encabezado['tipo_orden'] == 2){
                           $muestras_det = $mysqli->query("SELECT 
                                                                ...
                                                                ,ba.trn_id
                                                                ,om.folio_interno
                                                                ,ba.folio AS muestra_geologia
                                                          FROM  arg_ordenes_soluciones om 
                                                              LEFT JOIN `arg_ordenes_muestrasSoluciones` ba 
                                                              	ON om.trn_id_rel = ba.trn_id
                                                              WHERE 
                                                              	om.trn_id_batch = " . $trn_id . " AND om.metodo_id = " . $metodo_id . "
                                                               ORDER BY
                                                                    om.folio_interno"
                           ) or die(mysqli_error($mysqli));
                    }else{
                         if ($orden_encabezado['tipo_orden'] == 5){
                               $muestras_det = $mysqli->query("SELECT 
                                                                     om.trn_id_rel
                                                                    ,om.folio_interno
                                                                    ,om.muestra_geologia
                                                              FROM  
                                                                    ordenes_sobrelimites om 
                                                              WHERE 
                                                                  	om.trn_id_rel = ".$trn_id." AND om.metodo_id = 1
                                                              ORDER BY
                                                                    om.folio_interno"
                               ) or die(mysqli_error($mysqli));
                        }
                     ...
                     /*   else{
                            ...
                            if ($orden_encabezado['tipo_orden'] == 6){
                               $muestras_det = $mysqli->query("SELECT 
                                                                     om.trn_id_rel
                                                                     ,om.trn_id
                                                                    ,om.folio AS folio_interno
                                                              FROM  
                                                                    arg_ordenes_muestrasRecheck om 
                                                                    
                                                              WHERE 
                                                                  	om.trn_id_rel = ".$trn_id."
                                                              ORDER BY
                                                                    om.folio"
                               ) or die(mysqli_error($mysqli));
                        }*/
                        else{                           
                            if ($orden_encabezado['tipo_orden'] == 1 or $orden_encabezado['tipo_orden'] == 6) {
                                $muestras_det = $mysqli->query(
                                                                "SELECT
                                                                            ot.bloque,
                                                                            ot.posicion,
                                                                            ot.pos_geo,
