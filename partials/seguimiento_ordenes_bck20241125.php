...
<script>
  ...
  function iniciar_etapa(trn_id, etapa, unidad)
   ...
   function peso_guardar(trn_bat_sec, trn_rel_sec, cont_sec)
    ...
    function quebrado_guardar(trnid_batch, trnid_rel, contador)
    {
         ...
         if  (total_porc_que < 60 && comentario_que == '' && total_porc_que != '')  {   //Cambia a de 70 a 60
              warn_porc = 1;
              error_det = 'Si el porcentaje es menor que 70 debe especificar un comentario';
         }
    ...
    function iniciar_metodo(trn_id, metodo, fase, etapa, unidad)
    {
            ...
            // Agregado de nuevo el 20240910
            if (etapa == 19){//Pesaje Incuarte
                $('#incuarte_modal').modal('show');
                $("#datos_incuarte").html(respuesta);
                $('#incuarte_modal').on('shown.bs.modal', function (e) {
                    $(this).find('#peso_incuarte1').focus();
                    
                })                
                $('#incuarte_modal').modal('show').trigger('shown');
            }
            if (etapa == 20){//Peso Dore
                $('#dore_modal').modal('show');
                $("#datos_dore").html(respuesta);
                $('#dore_modal').on('shown.bs.modal', function (e) {
                    $(this).find('#peso_dore1').focus();
                })                
                $('#dore_modal').modal('show').trigger('shown');
            }
            if (etapa == 22){//Ataque Quimico EFAAG-30
                $('#ataque_modal').modal('show');
                $("#datos_Ataque").html(respuesta);
                $('#ataque_modal').on('shown.bs.modal', function (e) {
                    $(this).find('#hora_inicio').focus();
                })                
                $('#ataque_modal').modal('show').trigger('shown');
            }
            //// Agregado de nuevo el 20240910 ** FINAL**
            if (etapa == 23 || etapa == 24 || etapa == 25 || etapa == 26){//Titulacion soluciones
                $('#titulacion_modal').modal('show');
                $("#datos_titulacion").html(respuesta);
                $('#titulacion_modal').on('shown.bs.modal', function (e) {
                    $(this).find('#peso_tit').focus();
                })                
                $('#titulacion_modal').modal('show').trigger('shown');
            }
            if (etapa == 27){//Pesaje muestra
                //alert('ok');
                $('#metodo_modal').modal('show');
                $("#datos_metodo").html(respuesta); 
                $('#metodo_modal').on('shown.bs.modal', function (e) {
    ...
    //Metodo 1 EFGRA30 ORDEN SOBRELIMITES
    function iniciar_metodo_sobrelim(trn_id, metodo, fase, etapa, unidad)
    {
            trn_id = trn_id;
            metodo = metodo;
            fase   = fase;
            ...
            unidad_tem = unidad;
            document.getElementById("mina_met").value = unidad_tem;
            document.getElementById("mina_incuarte").value = unidad_tem;
            //alert('mesobreltodo');
            $.ajax({
            		url: 'iniciar_metodo_sobrelim.php',
            		type: 'POST' ,
            		dataType: 'html',
            		data: {trn_id:trn_id, metodo:metodo, fase:fase, etapa:etapa, unidad_tem:unidad_tem},
            }).done(function(respuesta){
            ...
            if (etapa == 5){//Pesaje muestra
                //alert('ok');
                $('#metodo_modal_sob').modal('show');
                $("#datos_metodo_sob").html(respuesta); 
                $('#metodo_modal_sob').on('shown.bs.modal', function (e) {
                    $(this).find('#peso_sob1').focus();
                })                
                $('#metodo_modal_sob').modal('show').trigger('shown');               
            }
            if (etapa == 8){//Fundicion
                $('#metodo_modal_sob').modal('show');
                $("#datos_metodo_sob").html(respuesta);
            ...
            if (etapa == 6){//Pesaje Payï¿½n
                $('#metodo_modal_sob').modal('show');
                $("#datos_metodo_sob").html(respuesta);
                $('#metodo_modal_sob').on('shown.bs.modal', function (e) {
                    $(this).find('#peso_soby1').focus();
                })                
                $('#metodo_modal_sob').modal('show').trigger('shown');
            }
            if (etapa == 19){//Pesaje Incuarte
            ...
            if (etapa == 10){
               revision_abs_sob(trn_id, metodo, unidad);
            }
            if (etapa == 11){
               enviar_resultados_sobr(trn_id, metodo, unidad, 1);
            }
            if (etapa == 12){
               enviar_resultados_sobr(trn_id, metodo, unidad, 0);
            }            
            if (etapa == 20){//Peso Dore
                $('#metodo_modal_sob').modal('show');
                $("#datos_metodo_sob").html(respuesta);
    ...
    //Metodos quebradora
    function iniciar_metodo_quebr(trn_id, metodo, fase, etapa, unidad)
    {
            trn_id = trn_id;
            metodo = metodo;
            fase   = fase;
            ...
            if (etapa == 10){
               revision_abs_quebr(trn_id, metodo, unidad);
            }
            if (etapa == 11){
               enviar_resultados_quebr(trn_id, metodo, unidad, 1);
            }
            if (etapa == 12){
               enviar_resultados_quebr(trn_id, metodo, unidad, 0);
            }
            if (etapa == 16){//Cianuracion
                $('#temperatura_modal').modal('show');
                $("#datos_temperatura").html(respuesta);
    ...
    // AGREGADO EL DIA 20241009
    function incuarte_guardar_glo()
    {
         var warn_met = 0;
         var trnid_orden   = document.getElementById("trnid_batch_met1").value;
         var metodo_id = 24;
         var fase = 9;
         var etapa = 19;
         var unidad = document.getElementById('mina_incuarte').value;
        // $('#boton_save_incGlob').html('<div class="loading"><i class="fa fa-spinner fa-spin fa-1x fa-fw"></i><span class="sr-only">Loading...</span></div>');
            /*alert(trnid_orden); 
            alert(unidad);
            alert(metodo_id);*/
                    $.ajax({
                		url: 'guardar_incuarte_glo.php' ,
                		type: 'POST' ,
                		dataType: 'html',
                		data: {trnid_orden:trnid_orden, metodo_id:metodo_id, fase:fase, etapa:etapa, unidad:unidad},
                    }).done(function(respuesta){
          ...
          // AGREGADO EL 20240910 PARA METODO EFAAG30
    function met_dore_guardar(trnid_batch, trnid_rel, metodo, fase, etapa, contador, unidad)
    {
         ...
         var etapa = etapa;
         var unidad = unidad;
         var table = document.getElementById("tabla_datos_dore");
         ...
         //Validar que el porcentajes sean mayores a 70   
         cantidad_met    = "peso_dore"+contador;
         ...
         if(cantidad_metodo == 0 || cantidad_metodo == '' || cantidad_metodo == 0.00)
            ...
            $('#boton_save_dore').html('<div class="loading"><i class="fa fa-spinner fa-spin fa-1x fa-fw"></i><span class="sr-only">Loading...</span></div>');
            //alert(cantidad_metodo); 
                    $.ajax({
                		url: 'guardar_met_dore.php' ,
                		type: 'POST' ,
                		dataType: 'html',
    ...
    function ataque_guardar(trn_id_ci, metodo_ci)
        {               
            var trn_id_agi    = trn_id_ci;
            var metodo_agi    = metodo_ci;
            var hora_ini  = document.getElementById("hora_inicio_at").value;
            var hora_fin  = document.getElementById("hora_final_at").value;
            var fase_id_agi = 8;
            var etapa_id_agi = 22;
    ...
    function met_titulacion_guardar(trnid_batch, trnid_rel, metodo, fase, etapa, contador, unidad)
    {
         var contador = contador;   
         var warn_met = 0;
         var trnid_orden   = trnid_batch;
         ...
         var etapa = etapa;
         var unidad = unidad;
         var table = document.getElementById("tabla_titulacion");
         var total_rows_m = parseInt(table.rows.length)-3;
         //alert(total_rows_m);
         //$('#guardando_modal_peso').modal('show');
         //alert(total_rows_m);
         ...
         //alert(fin_met)
         //Validar que el porcentajes sean mayores a 70   
         cantidad_met    = "peso_tit"+contador;
         cantidad_metodo = document.getElementById(cantidad_met).value;
        // alert(cantidad_metodo);
         if(cantidad_metodo == 0 || cantidad_metodo == '' || cantidad_metodo == 0.00)
            {
           ...
           // cantidad_metodo = document.getElementById(cantidad_met).value;   
            ...
            //$('#peso_met1') = '<img src="images/upload.gif" alt="loading" />';
            //alert(trnid_orden);alert(trnid_muestra);alert(metodo_id);  alert(fase);alert(etapa);  alert(cantidad_metodo);  
            $('#boton_save_tit').html('<div class="loading"><i class="fa fa-spinner fa-spin fa-1x fa-fw"></i><span class="sr-only">Loading...</span></div>');
            //alert(fin_met); 
                    $.ajax({
                		url: 'guardar_tit_peso.php' ,
                		type: 'POST' ,
                		dataType: 'html',
                		data: {trnid_orden:trnid_orden, trnid_muestra:trnid_muestra, metodo_id:metodo_id, fase:fase, etapa:etapa, cantidad_metodo:cantidad_metodo, fin_met:fin_met, unidad:unidad},
                    }).done(function(respuesta){
    ...
    function exportar_absorcion(trn_id_abs, metodo_abs, u_id_exp)
            {
                 ...
                 if(metodo_abs == 33 || metodo_abs == 27 || metodo_abs == 28){
                    var exportar = '<?php echo "\ exportar_absatomica_cian.php?trn_id_a="?>'+trn_id_a+'&metodo_id_a='+metodo_id_a+'&u_id_a='+uid_exp; 
                 }
                 else{
    ...
    function revision_abs_sob(trn_id_abso, metodo_abso, unidad)
            {
                 var trn_id_absat    = trn_id_abso;
                 var metodo_id_absat = metodo_abso;
                 var unidad_id = unidad;
                     var exportar = '<?php echo "\ revision_absatomica_sobr.php?trn_id_abs="?>'+trn_id_absat+'&metodo_id_abs='+metodo_id_absat+'&unidad_id='+unidad_id;                                  
                     window.location.href = exportar;
            }
            
    function revision_abs_quebr(trn_id_abso, metodo_abso, unidad)
            {
                 var trn_id_absat    = trn_id_abso;
                 var metodo_id_absat = metodo_abso;
                 var unidad_id = unidad;
                     var exportar = '<?php echo "\ revision_absatomica_quebr.php?trn_id_abs="?>'+trn_id_absat+'&metodo_id_abs='+metodo_id_absat+'&unidad_id='+unidad_id;                                  
                     window.location.href = exportar;
            }
    ...
    //Guardar ataque quimico sobrelimites
    function ataque_guardar_sobr(trn_id_ci, metodo_ci)
        {               
            var trn_id_agi    = trn_id_ci;
            var metodo_agi    = metodo_ci;
            var hora_ini  = document.getElementById("hora_inicio_at").value;
    ...
    function met_payon_guardar(trnid_batch, trnid_rel, metodo, fase, etapa, contador, unidad)
    {
         ...
         if(cantidad_met_pay == 0 && cantidad_met_pay == '' && edicion == 0)
            ...
            if (cantidad_met_pay < 20 || cantidad_met_pay > 45){
    ...
    function met_payon_guardarEdit(trnid_batch, trnid_rel, metodo, fase, etapa, contador, unidad)
    {
    ...
    function met_payon_finalizar(trnid_batch, metodo, fase, etapa)
    {
        ...
        $('#boton_save_payconfirm').html('<div class="loading">FINALIZANDO ETAPA...<i class="fa fa-spinner fa-spin fa-2x fa-fw"></i><span class="sr-only">Finalizando...</span></div>'); 
                $.ajax({
                		url: 'guardar_payon_confirmar.php' ,
                		type: 'POST' ,
                		dataType: 'html',
    ...
    function copelado_guardar(trn_id_cop, metodo_cop)
        {               
            ...
            {
                if(cantidad_cp < 920 || cantidad_cp > 950){
                    alert('Error: Temperatura fuera de rango, reintente por favor');
                }
                else{
    ...
    //Importar satisfactoriamente
    function ver_csv_quebr(archivo, metodid_importar)
        {         
               var archivo_imp = archivo;
               var unidad_id_abs =  document.getElementById("mina_absimp").value;  
               document.getElementById("mina_abs_esp").value = unidad_id_abs; 
               $.ajax({
                		url: 'leer_csv_quebr.php' ,
                		type: 'POST' ,
                		dataType: 'html',
                		data: {archivo_imp:archivo_imp},
                	})
    ...
    function enviar_resultados_sobr(trn_id_env, metodo_id_env, unidad_id, pree)
        {         
               ...
               //alert(pre);  
                enviar_notificacion(trn_id_env, metodo_id_env, pre);      
               var print_d = '<?php echo "\liberar_orden_sobr.php?trn_id_a="?>'+trn_id_env+'&metodo_id_a='+metodo_id_env+'&pree='+pre;        
               window.location.href = print_d;
        }
    
     function enviar_resultados_quebr(trn_id_env, metodo_id_env, unidad_id, pree)
        {         
               var trn_id_env = trn_id_env;
               var metodo_id_env = metodo_id_env;
               var pre = pree;    
               //alert(pre);  
               enviar_notificacion(trn_id_env, metodo_id_env, pre);      
               var print_d = '<?php echo "\liberar_orden_quebr.php?trn_id_a="?>'+trn_id_env+'&metodo_id_a='+metodo_id_env+'&pree='+pre;        
               window.location.href = print_d;               
        }
        
     function descargar_cert(trn_id_env)
        {         
               var trn_id_env = trn_id_env;
               
               var print_d_pdf = '<?php echo "\certificado_pdf.php?trn_id="?>'+trn_id_env;   
               window.location.href = print_d_pdf;     
        }
   ...
   function enviar_notificacion(trn_id_li, metodo_id_li, pree)
        {    
                ...
                $.ajax({
            		...
            		type: 'POST' ,
            		dataType: 'html',
            		data: {trn_id_l, trn_id_l, metodo_id_l,metodo_id_l, preel},
            	})
            	.done(function(respuesta){
   ...
   function met_sobr_guardar(trnid_folio, trnid_batch, trnid_rel, metodo, fase, etapa, contador, unidad)
    {
         var contador = contador;   
         var warn_met = 0;
         var trnid_folio   = trnid_folio;
         var trnid_orden   = trnid_batch;
         var trnid_muestra = trnid_rel;
         var metodo_id = metodo;
         var msg_err = '';
         ...
         var etapa = etapa;
         var unidad = unidad;
         var table = document.getElementById("tabla_pesaje_sobr");
         var total_rows_m = parseInt(table.rows.length)-3;
         //alert(contador);
         //alert(total_rows_m);
         if (total_rows_m == contador){
         ...
         //Validar que el porcentajes sean mayores a 70   
         cantidad_met    = "peso_sob"+contador;
         cantidad_metodo = document.getElementById(cantidad_met).value;
         //alert(cantidad_metodo);
         if(cantidad_metodo == 0 || cantidad_metodo == '' || cantidad_metodo == 0.00)
            {
               warn_met = 1;
               msg_err = 'El valor debe ser diferente de cero. Reintente por favor';
            }
         if(etapa == 5 && cantidad_metodo > 50)
            {
                warn_met = 1;
                msg_err = 'Peso fuera de rango, el valor debe ser menor que 50. Reintente por favor';
            }
         if(etapa == 6 && cantidad_metodo < 20){
                warn_met = 2;
                msg_err = 'Peso fuera de rango. Se enviará la muestra a REENSAYE';
         }
         
         if(etapa == 6 && cantidad_metodo > 45){
                warn_met = 2;
                msg_err = 'Peso fuera de rango. Se enviará la muestra a REENSAYE';
         }
        ...
        if (warn_met == 1){
            alert(msg_err);
               ...
               if (warn_met == 2){
                ...
                //alert(fin_met); 
                        $.ajax({
                	       	url: 'guardar_sobr_peso.php' ,
                    		type: 'POST' ,
                    		dataType: 'html',
                    		data: {trnid_folio:trnid_folio, trnid_orden:trnid_orden, trnid_muestra:trnid_muestra, metodo_id:metodo_id, fase:fase, etapa:etapa, cantidad_metodo:cantidad_metodo, fin_met:fin_met, unidad:unidad},
                        }).done(function(respuesta){
                        if(respuesta == 'Ha finalizado la etapa.')//alert(respuesta);  
                        {
                            alert(respuesta);
 ...
 function iniciar_batch_recheck (trn_id, unidad)
        {               
            trn_id = trn_id;
            unidad_id = unidad;
            //alert(trn_id);
...
<div class="modal fade" data-backdrop="static" data-keyboard="false" id="ataque_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
   <div class="modal-dialog modal-dialog-scrollable" role="document">
            <div class="modal-content">
              <div class="modal-header" >
                <h5 class="modal-title" id="nombre_etapa">METODOS</h5>                             
              ...
              <div class="modal-body"  style="font-size:4px;" class="col-md-12 col-lg-12" id="datos_Ataque">
              </div>
              <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" onclick="actualizar_met();" data-dismiss="modal">Cerrar</button>
              </div>
...
<div class="modal fade" data-backdrop="static" data-keyboard="false" id="dore_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
   <div class="modal-dialog modal-dialog-scrollable" style="max-width: 750px!important;"  role="document">
            <div class="modal-content">
              <div class="modal-header" >
                <h5 class="modal-title" id="nombre_etapa">METODOS</h5>                             
                <div class="col-md-1 col-lg-1">                
                    <input type="date" id="fecha_metodo"  value="<?php echo date("Y-m-d");?>" min="<?php echo date("Y-m-d");?>" disabled />                    
                    <input type="hidden" id="mina_dore" size=20 style="width:200px; color:#996633" value="<?php echo $unidad_id;?>" disabled /> 
                </div>
              </div> 
              <div class="modal-body"  style="font-size:4px;" class="col-md-12 col-lg-12" id="datos_dore">
              </div>
              <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" onclick="actualizar_met();" data-dismiss="modal">Cerrar</button>
              </div>
              ...
              if ($action == "upload") //si action tiene como valor UPLOAD haga algo (el value de este hidden es es UPLOAD iniciado desde el value
                ...
                {       
                    ...
                    if($archivo_ex == $archivo){
                        ...
                        echo "<script> ver_csv_quebr('$archivo');
                        $('#progress_modal').modal('show');
                        </script>";
                    }              
                    else{
...
<div class="modal fade" data-backdrop="static" data-keyboard="false" id="metodo_modal_sob" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
   <div class="modal-dialog modal-dialog-scrollable" style="max-width: 750px!important;" role="document">
            <div class="modal-content">
              <div class="modal-header" >
                <h5 class="modal-title" id="nombre_etapa">METODOS  SOBRELIMITES</h5>                             
                <div class="col-md-1 col-lg-1">                
                    <input type="date" id="fecha_metodo"  value="<?php echo date("Y-m-d");?>" min="<?php echo date("Y-m-d");?>" disabled />                    
                    <input type="hidden" id="mina_met" size=20 style="width:125px; color:#996633"  disabled /> 
                </div>
              </div> 
              <div class="modal-body"  style="font-size:4px;" class="col-md-12 col-lg-12" id="datos_metodo_sob">
              </div>
              <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" onclick="actualizar_met();" data-dismiss="modal">Cerrar</button>
              </div>
...
<div class="modal fade" data-backdrop="static" data-keyboard="false" id="incuarte_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
   <div class="modal-dialog modal-dialog-scrollable" style="max-width: 750px!important;"  role="document">
            <div class="modal-content">
              <div class="modal-header" >
                <h5 class="modal-title" id="nombre_etapa">METODOS</h5>                             
                <div class="col-md-1 col-lg-1">                
                    <input type="date" id="fecha_metodo"  value="<?php echo date("Y-m-d");?>" min="<?php echo date("Y-m-d");?>" disabled />                    
                    <input type="text" id="mina_incuarte" size=20 style="width:200px; color:#996633" value="<?php echo $unidad_id;?>" disabled /> 
                </div>
              </div> 
              <div class="modal-body"  style="font-size:4px;" class="col-md-12 col-lg-12" id="datos_incuarte">
              </div>
              <div class="modal-footer">
                    <button type="button" class="btn btn-primary" id="boton_save_incGlob" onclick="incuarte_guardar_glo();" data-dismiss="modal" >
                    GUARDAR</button>
                    <button type="button" class="btn btn-secondary" onclick="actualizar_met();" data-dismiss="modal">Cerrar</button>
              </div>
            </div>
...
<div class="modal fade" data-backdrop="static" data-keyboard="false" id="titulacion_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
   <div class="modal-dialog modal-dialog-scrollable" style="max-width: 750px!important;"  role="document">
            <div class="modal-content">
              <div class="modal-header" >
                <h5 class="modal-title" id="nombre_etapa">METODO</h5>                             
                <div class="col-md-1 col-lg-1">                
                    <input type="date" id="fecha_metodo"  value="<?php echo date("Y-m-d");?>" min="<?php echo date("Y-m-d");?>" disabled />                    
                    <input type="text" id="mina_incuarte" size=20 style="width:200px; color:#996633" value="<?php echo $unidad_id;?>" disabled /> 
                </div>
              </div> 
              <div class="modal-body"  style="font-size:4px;" class="col-md-12 col-lg-12" id="datos_titulacion">
              </div>
              <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" onclick="actualizar_met();" data-dismiss="modal">Cerrar</button>
              </div>
                  ...
                  $html_det = "<table class='table table-striped' id='motivos'>
                                <thead>                                
                                     <tr class='table-info'>      
                                        ...
                                        <th colspan='3' center>Listado</th>";
                                    ...
                                    <tr class='table-info' justify-content: center;>            
                                        ...
                                        <th colspan='3'>Hoja Preparación</th>";
                                   /*     if ($imprime_etiquetas <> 0){
                                            $html_det = "<th scope='col1'>Controles</th>"; 
                                            $html_det = "<th scope='col1'>Etiquetas</th>";      
                                        }   */    
                               ...
                               while ($fila = $datos_orden_detalle->fetch_assoc()) {
                                   ...
                                   $html_det.="<tr>";
                                      ...
                                      if ($fila['estado_id'] == 0 and $fila['reensaye'] == 1 and $fila['prepara'] > 0 ){
                                        $html_det.="<td> <a href='orden_trabajo_mover.php?trn_id=".$fila['trn_id']."' target='_blank'>".$fila['folio_interno']."</a></td>";
                                      }
                                      else{
                                                 ...
                                                 $html_det.="<td><a type='button' class='btn btn-warning' name='prinT' id='print'";
                                                                if($fila['reensaye'] == 0){
                                                                    if ($fila['recheck'] == 0){
                                                                       ...
                                                                       // if ($fila['prepara'] > 0 and $fila['iniciar_reen'] == 1){ Se quita el 20240912
                                                                        if ($fila['prepara'] > 0 ){
                                      ...
                                      if ($fila['reensaye'] == 1){
                                          ...
                                          elseif ($fila['estado_id'] <> 0 ){
                                                if($fila['metalurgia'] == 3){
                                                     $html_det.="<td> <a button type='button' class='btn btn-success' href='orden_trabajo_muestrasMet.php?trn_id=".$fila['trn_id_batch']."&metodo_id=".$fila['metodo_id']."&unidad_id=".$unidad_id."' target='_blank'>
                                                                        <span class='fa fa-flask fa-2x'></span>
                                                                     </button></td>";
                                                }
                                                else{
                                    ...
                                    if ($fila['prepara'] > 0){   
                                        if($fila['metalurgia'] == 3){
                                                    $html_det.="<td> <a button type='button'class='btn btn-warning' href='orden_trabajo_muestrasControl_Met.php?trn_id=".$fila['trn_id_batch']."&metodo_id=".$fila['metodo_id']."'&unidad_id=".$unidad_id."' target='_blank'>
                                                                        <span class='fa fa-flask fa-2x'></span>
                                                                     </button></td>";   
                                                }
                                                else{
                                        ...
                                        $html_det.="<td> <a button type='button'class='btn btn-info' href='hoja_preparacion.php?trn_id=".$fila['trn_id']."' target='_blank'>
