            ...
            if ($resultado->num_rows > 0) {
                while ($res = $resultado->fetch_assoc()) {
                 ...
                 if ($tipo_can == 1){   //Porcentajes
                        $limite = round((($cantidad_muestras*$total)/100)); 
                  }
                 /* if ($tipo_can == 2){
                 ...
                 $resultado_mues = $mysqli->query("SELECT * FROM  (SELECT
                                                    ...
                                                    WHERE ot.tipo_id = 0  
                                                          ...
                                                          AND sec.peso = 0
                                                    ORDER BY posicion
                                                    LIMIT ".$limite.") AS x ORDER BY posicion") or die(mysqli_error());
                                                    
                                                    /* Para que sean posicones alaeatorias: ORDER BY (FLOOR (1+RAND()*".$total."))
                                                    LIMIT ".$limite.") AS x ORDER BY bloque, posicion"*/
