...
$datos_at = $mysqli->query("SELECT od.folio_interno, (CASE WHEN o.trn_id_rel <> 0 THEN 1 ELSE 0 END ) AS reensaye, o.tipo
...
$soluciones = $usuario_atiende['tipo'];


        //echo $tope_ree;
        $datos_met = $mysqli->query("SELECT nombre AS nombre_metodo, elemento FROM arg_metodos WHERE metodo_id = $metodo_id_a") or die(mysqli_error($mysqli));
...
if($soluciones != 2){
     ...
     mysqli_multi_query($mysqli, "CALL arg_prc_revisionAbsorcionSoluciones ($trn_id_a, $metodo_id_a)") or die(mysqli_error($mysqli));
                   $objSheet->setTitle($nombre_metodo);
                   $spreadsheet->setActiveSheetIndex(0) 
                            ->setCellValue('A1', 'FECHA')
                            ->setCellValue('B1', 'MUESTRA')                    
                            ->setCellValue('C1', $elemento)             
                            ->setCellValue('D1', 'METODO FINAL')
                            ->setCellValue('E1', 'FECHA RESULTADO');
                       //     ->setCellValue('I1', 'HORA');
            
                    //$spreadsheet->getActiveSheet(0)->setTitle($nombre_metodo.'_SOLUCIONES');
                   // $objPHPExcel->setActiveSheetIndex(1);
                ...
                if ($result = mysqli_store_result($mysqli)) {                
                        while ($row = mysqli_fetch_assoc($result)) {
                        $spreadsheet->setActiveSheetIndex(0) 
                        ->setCellValue('A'.$i, $row['fecha_orden'])
                        ->setCellValue('B'.$i, $row['folio'])
                        ->setCellValue('C'.$i, $row['resultado'])
                        ->setCellValue('D'.$i, $row['nombre'])
                        ->setCellValue('E'.$i, $row['fecha_fin']);                        
                        $i=$i+1;
                    }  
                }
      }
