...
$tipo = $orden_encabezado['tipo'];

if ($tipo == 6){
    $detalle_etiquetas = $mysqli->query("SELECT 
                                                 o.folio AS orden 
                                                ,(CASE WHEN ot.tipo_id = 0 THEN om.folio ELSE ot.control END) AS muestra_geologia   
                                                ,ot.folio_interno
                                                ,orden_ensaye(om.trn_id) AS ensaye
                                            FROM
    ...
    $detalle_etiquetas = $mysqli->query("SELECT 
                                            ...
                                            INNER JOIN ordenes_transacciones otr
            ...
            <table border='0' width='98%' CELLPADDING=5 CELLSPACING=0>
                             <thead>
                                <tr>
                                 ...
                                 <tr>
                                    <th width='80%' align='left'>METODO: DRY_550, CRU_60 y PUL_80</th>
                                 </tr>
                                 <tr>
...
$html_en .= "<table border = '1' align='center' width='98%' CELLPADDING=5 CELLSPACING=0>
                            <thead>
                                <tr>
                                    <th width='80% align='center' style='border:none;'> RECEPCION:__________</th>
                                    <th width='80% align='center' style='border:none;'> SECADO:__________</th>
                                    <th width='80% align='center' style='border:none;'> QUEBRADO:__________</th>
                                </tr>
                </thead></table>
                <table border = '1' align='center' width='98%' CELLPADDING=5 CELLSPACING=0>
                                <tr>
                                    <th width='24% align='center' colspan=4> PULVERIZADO:____________________</th> 
                                ";
...
while ($fila_det = $detalle_etiquetas->fetch_assoc()) {
    ...
    if($tipo == 6){
            $etiqueta_ensaye[$f]  = $fila_det['ensaye'];
    }    
    $f++;
}
...
$ch = 1;
$page = 1;
/*$total_paginas = ceil($total_muestras / 23);
$total_filas   = ceil($total_muestras / 23);*/
$total_paginas = ceil($total_muestras / 22);
$total_filas   = ceil($total_muestras / 22);
$total = $total_muestras;
while ($page <= $total_paginas) {
    $html_det .= "<div class='row' style='margin-left: 1%; margin-top: 52%;'>";
    $columna = 1;
    while ($columna < 3 && $i <= $total_muestras) {
       ...
       if($tipo == 6){
            $html_det .= "<th align='center' colspan='2'>ENSAYE</th>";
       }
       $html_det .= "</tr></thead><tbody>";
        while ($total > 0) {
            ...
            $html_det .= "<tr>
                                ...
                                if($tipo == 6){
                                    $html_det .= "<th colspan='2'>" . $etiqueta_ensaye[$i] . "</th>"; 
                                }
                            $html_det .= "</tr>";
            $i = $i + 1;
            $f = $f + 1;
...
$pdf_header .= '<img width="' . (floor($iwidth * $img_factor)) . '" height="' . (floor(($iwidth * $ifactor) * $img_factor)) . '" src="http://172.16.52.58/lcminedatalabs/images/encabezado_prep.jpg">';
